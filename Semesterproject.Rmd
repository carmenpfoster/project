...
On Figure 1 one can see the whole study area and the two locations of the weather stations.
...
As it can be seen in Figure ?? the wild boars with the most measurements are Caroline, Amos and Miriam. The locations of those three wild boars were then plotted in a map with the Swiss orthofoto as the basemap. Then the points were classified depending on the season, temperature, precipitation or the ratio of static points and searched for interessting patterns.
...
ggplot(wildschwein_dailysummary, aes(x = season))+
  ...
  labs(title = "Number of measurement days per season", subtitle = "per wild boar", caption = " Figure ???: Barplots with # of measurement days per wild boar split in the 4 seasons")+
  ylab("# of tracked days")+
  theme(plot.caption = element_text(hjust = 0))
```

In a first way, we wanted to find out how many measurement days per seasons and per wild boar were recorded. One can see that the wild boar caroline has the most measurements over all seasons compared to the other wild boars. Some wild boars do not even have measurements for some seasons, such as Isabelle who was only tracked days for the seasons autumn and winter.

```{r}
#boxplot daily distance season
ggplot(wildschwein_dailysummary, aes(x = season, y = daily_distance)) + 
  geom_boxplot()+
  labs(title = "Distance travelled per day", subtitle = "per wild boar", caption = " Figure 4: Boxplot of calculated distances per day and wild boar split up in the seasons")+
  ylab("distance [m]")+
  theme(plot.caption = element_text(hjust = 0))

```

This boxplot represents the daily distance, how far the wild boars moved, in each season without including the temperature. One can see that the daily distance is farer in autumn and summer compared to winter especially.


```{r}
#anova season - dailydistance
...
```
Kommentar für Vibiga: Sommer und Herbst kein statistisch signifikanter Unterschied, zwischen allen anderen Gruppen gibt es signifikanten Unterschied
...
ggplot(wildschwein_dailysummary, aes(x=season, y = ratio))+
  geom_boxplot()+
  labs(title = "Share of static measurement points per day", subtitle = "per wild boar", caption = "Figure 5: Boxplot of the ratio of static measurements per wild boar and day split up into the 4 seasons")+
  ylab("ratio")+
  theme(plot.caption = element_text(hjust = 0))
```
...
```
Kommentar für Vibiga: Unterschied zwischen Frühling/Herbst, Sommer/Frühling und Winter/Frühling statistisch nicht signifikant, die Unterschiede der anderen "Paare" sind statistisch signifikant
...
ggplot(wildschwein_dailysummary, aes(x=temp, y=daily_distance, col = season)) + 
  ...
  labs(title = "Influence of temperature on the travelled distance per day", subtitle = "per season", caption = "Figure 6: Scatterplot of mean temperature and travelled distance")+
  xlab("temperature [°C]")+
  ylab("distance [m]")+
  theme(plot.caption = element_text(hjust = 0))
```
...
ggplot(wildschwein_dailysummary, aes(x=temp, y=ratio, col = season)) +
  geom_point(size=0.8)+
  geom_smooth(method=lm)+
  labs(title = "Influence of temperature on the share of static measurement points", subtitle = "per season", caption = "Figure 7: Scatterplot of the mean temperature and the ratio of static measurements")+
  xlab("temperature [°C]")+
  ylab("distance [m]")+
  theme(plot.caption = element_text(hjust = 0))
```
...
ggplot(wildschwein_dailysummary, aes(x=temp, y=daily_distance, col = season)) +
  geom_point()+
  labs(title = "Influence of temperature on the travelled distance per day", subtitle = "per season for all wild boars seperately", caption = "Figure 8: Scatterplot of mean temperature and travelled distance for every wild boar seperately")+
  xlab("temperature [°C]")+
  ylab("distance [m]")+
  theme(plot.caption = element_text(hjust = 0))+
  facet_wrap(~TierName,labeller = label_value)
...
ggplot(wildschwein_dailysummary, aes(x=precip, y=daily_distance, col = season)) + 
  ...
  labs(title = "Influence of precipitation on the travelled distance per day", subtitle = "per season", caption = "Figure 9: Scatterplot of daily precipitation and travelled distance")+
...
ggplot(wildschwein_dailysummary, aes(x=precip, y=ratio, col = season)) +
  geom_point(size=0.8)+
  geom_smooth(method=lm)+
  labs(title = "Influence of precipitation on the share of static measurement points", subtitle = "per season", caption = "Figure 10: Scatterplot of daily precipitation and the ratio of static measurement points per wild boar")+
  xlab("precipitation [mm]")+
  ylab("static ratio")+
  theme(plot.caption = element_text(hjust = 0))
```
...
ggplot(wildschwein_dailysummary, aes(x=precip, y=daily_distance, col = season)) +
  geom_point()+
  labs(title = "Influence of precipitation on the travelled distance per day", subtitle = "per season for all wild boars seperately", caption = "Figure 11: Scatterplot of precipitation and travelled distance for every wild boar seperately")+
  xlab("precipitation [mm]")+
  ylab("distance [m]")+
  theme(plot.caption = element_text(hjust = 0))+
  facet_wrap(~TierName,labeller = label_value)
